...
export const AnswersDashboard = ({ rounds, answers, onJudge, onJudgeBulk, activeQuestionId, totalParticipants }: Props) => {
    ...
    const groups = useMemo<AnswerGroup[]>(() => {
        ...
        // Most popular answer first — ordered by how many teams wrote it,
        // not by similarity to the accepted answer ("matches accepted" is
        // just a badge that can land on any group). This re-sorts live as
        // answers come in, so a group's spot can move as it grows; judging
        // a group doesn't change its size, so judging alone never reorders.
        result.sort((a, b) => {
